...
function api_host_url_without_offices (office, url) {
    if (!office) return "cloud/"+url;
    // var t=(office.lat+"c"+office.lon).replace(/\./g,'d').replace(/-/g,'n');
    return "offices/"+url;
}
...
var apiHost={
    search: function (index, queries, office, size) {
        if (typeof size=="undefined") size=10000;
        var url = api_host_url(office, "api/search");
        console.log("Url", url);
        var args={index:index,queries:queries,size:size};
        return $.get(url,args);
    },
    search_without_offices: function (index, queries, office, size) {
        if (typeof size=="undefined") size=25;
        var url = api_host_url_without_offices(office, "api/search");
        console.log("Url", url);
        var args = { index: index, queries:queries, size:size};
        return $.get(url, args);
    },
