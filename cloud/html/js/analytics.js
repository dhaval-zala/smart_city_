...
function draw_analytics(video, doc) {
    ...
    var colors_label={ "person": "red", "vehicle": "cyan", "bike": "lime", "Covered_Face": "red", "Covid_Mask": "cyan" };
    var colorr="red"
        ...
        var draw=function () {
            ...
            if (tid!=video.data('last_draw')) {
    		    ...
    		    // console.log("------------------printing------------------", v._source);
                            ...
                            if ("detection" in v1 && v1.roi_type=="person") {
                                if ("bounding_box" in v1.detection) {
                                    ...
                                    if (xmin!=xmax && ymin!=ymax) {
                                        ...
                                        svghtml = svghtml + '<rect x="' + (sxoff + xmin) + '" y="' + (syoff + ymin) + '" width="' + (xmax - xmin) + '" height="' + (ymax - ymin) + '" stroke="' + colors_label[v1.detection.label] + '" stroke-width="1" fill="none"></rect><text x="' + (sxoff + xmin) + '" y="' + (syoff + ymin) + '" fill="cyan" font-size = "large">' + (text.translate(v1.detection.label) + id + text.translate(" : ")+Math.floor(v1.detection.confidence*100)+"%")+'</text>';
                                    }
                                }
                            }
                        });
                    ...
                    // roi = [[0, 500, 0, 1080, 950, 1080, 950, 500, 0], [125, 150, 125, 500, 650, 500, 650, 150, 0]]
                    roi = [[775.2, 656, 1149.6, 700.8, 1253.6, 438.4, 940, 283.2, 776.8, 662.4, 780, 660.8, 775.2, 656]]
                    for (i = 0; i < roi.length; i++) {
                        ...
                        var colorr = "cyan"
                        svghtml = svghtml + '<polygon points="' + xa1 + ',' + ya1 + ',' + xa2 + ',' + ya2 + ',' + xa3 + ',' + ya3 + ',' + xa4 + ',' + ya4 + '" stroke="' + colorr + '" stroke-width="3" fill="none"></polygon>';
