$("#pg-recording").on(":initpage", function(e, queries, office) {
        ...
        console.log("Inside the aivid");
        /* enable recording queries */
        var monitor = 0;
   
        
        $("#Selected").data('index', 'recordings').data('office', office).data('invoke', function (queries) {
            var plist = page.find("[play-list]");
            plist.empty();
            console.log("Inside the jquery")
            apiHost.search($("#Selected").data('index'), queries, office).then(function (data) {
        ...
        $("#sensorSelected").val(queries).focus().trigger($.Event("keydown", { keyCode: 13 }));
