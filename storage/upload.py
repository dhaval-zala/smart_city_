...
class UploadHandler(web.RequestHandler):
    ...
    def _rec2db(self, office, sensor, timestamp, path):
        ...
        if disk_usage<halt_rec_th:
            ...
            dbs = DBCommon(host=dbhost, index="sensors", office=office)
            sensor_index = dbs.get(sensor)
            algorithm = sensor_index["_source"]["algorithm"]
            sensor_name = sensor_index["_source"]["name"]
            ...
            mp4path = self._storage+"/"+node_name+"/"+sensor_name+"/"+algorithm+"/"+str(dt.year)+"/"+str(dt.month)+"/"+str(dt.day)
        ...
        if sinfo:
            ...
            db_rec_m = DBIngest(host=dbhost, index="recordings", office="")
            db_rec_m.ingest(sinfo)
