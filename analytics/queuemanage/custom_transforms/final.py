...
print("mqtthostmqtthost",mqtthost, mqttport, static_ip)
...
class QueueCounting:
    ...
    def new_initmethod(self, BOTNAME):
        ...
        for botconfig_id in botconfig_ids:
            ...
            _activity_id = botconfig_index["_source"]["activityId"]
            ...
            activities_index = _dba.get_without_office(_activity_id)
            activity_name = activities_index["_source"]["uniqueName"]
            ...
            for zones in dbz.search_without_office("botName:'"+str(BOTNAME)+"' and sensorId:'"+str(sensor_ids[0])+"'"):
                coordinate = zones["_source"]["coordinates"][0] #replace here
            ...
            for notification_responsetype_id in responsetype_ids:
                ...
                responce_p = responsetype["_source"]["trueResponse"]["name"]
                responce_n = responsetype["_source"]["falseResponse"]["name"]

                inspectionScores_p.append(inspectionScore_p)
                inspectionScores_n.append(inspectionScore_n)
                responces_p.append(responce_p)
                responces_n.append(responce_n)

            print("inspectionScores",inspectionScores_p, flush=True)
            print("inspectionScores",inspectionScores_n, flush=True)
            print("responces_p",responces_p)
            print("responces_n",responces_n)
            ...
            for sensors_num in range(len(sensor_ids)):
                ...
                for rois_num in range(len(coordinates)):
                    ...
                    for num_notification in range(len(responsetype_ids)):
                        ...
                        text_message = texts[num_notification]
                        ...
                        _notifications.append({
                            ...
                            "other_data":{
                                ...
                                "responces_n":responces_n[num_notification],
                                "responces_p":responces_p[num_notification],
    ...
    def send_notification_without_snap(self,to, text_message):
        ...
        try:
            notification(mqtthost,mqttport,json_msg1)
            # notification("stadium-office1-mqtt-service.default.svc.cluster.local",1883,json_msg1)
            print("send_notification_without_snap_0",json_msg1)
        except Exception as error:
            print("send_notification_without_snap_1",error)

    def send_notification_with_snap(self,to, node_name, sensor_id, botname, timestamp, text_message):
        temp_path = "http://"+static_ip+"/images/"+str(node_name)+"/"+str(sensor_id)+"/"+str(botname)+"/"+"img_"+str(timestamp)+".jpeg"
        # temp_path = "http://"+static_ip+"/images/"+str(node_name)+"/"+str(sensor_id)+"/"+str(botname)+"/"+"img_"+str(timestamp)+".jpeg"
        ...
        try:
            ...
            print("send_notification_without_snap_3",error)
    ...
    def process_frame(self,frame):
        ...
        def final_json_data(botconfig_id, sensor_id, zone_name, notification_id):
            final_json_data = []
        ...
        for botconfig_data in new_data["botconfig_data"]:
            ...
            activity_name = botconfig_data["activity_name"]
                            ...
                            for zone_ in i["roi"]:
                                ...
                                for m in list(detected_centers.keys()):
                                    ...
                                    try:
                                        ...
                                        print("wrong in tracker:{}".format(error),flush=True)
                                    print("notification__infoinfoinfoinfo",info)
                                    try:
                                        if info==True:
                                            object_count += 1
                                            ...
                                            if zone_cnt in self.queue[botconfig_id][self.camera_id][zone_cnt].entry and zone_cnt not in self.queue[botconfig_id][self.camera_id][zone_cnt].exit:
                                                self.tracker.end_time[zone_cnt]=time 
                                ...
                                for notifications_  in zone_["notification"]:
                                    try:
                                        ...
                                        print("notification_idnotification_id",notification_id)
                                        ...
                                        responce_p = sensor_other_data["responces_p"]
                                        responce_n = sensor_other_data["responces_n"]
                                        ...
                                        print("real_sendreal_snapshot",real_send, real_snapshot)
                                        
                                        to = sensor_other_data["to"]

                                        try:
                                            print("notification__",botconfig_id,notification_id, self.camera_id, zone_name, notification_id)
                                            new = final_json_data(botconfig_id, self.camera_id, zone_name, notification_id)
                                        except Exception as error:
                                        ...
                                        self.compliance_capture[botconfig_id][self.camera_id][zone_cnt][notification_id]["coordinates"] = self.queue[botconfig_id][self.camera_id][zone_cnt].roi
                                        
                                        if new == True:
                                            inspection_cnt += 1
                                            if self.total_count[botconfig_id][self.camera_id][zone_cnt][notification_id] > int(exe):
                                                score = inspectionScore_n
                                                responce = responce_n
                                                compliance = False
                                                imgpath = "http://"+static_ip+"/images/"+str(node_name)+"/"+str(self.camera_id)+"/"+str(BOTNAME)+"/"+"img_"+str(_time)+".jpeg"
                                            ...
                                            if inspection_cnt == 1:
                                                ...
                                                if start_time == None:
                                                    json_msg["new_time"] = _time * 1000
                                                else:
                                                    
                                                    json_msg["new_time"] = start_time + (_time-int(time_json()))
                                                ...
                                                json_msg["inspectionScore"] =score
                                            ...
                                            try:
                                                ...
                                                text_message = text_message.replace("<CHECKLIST NAME>", str("WRK-COM"))
                                                text_message = text_message.replace("<ACTIVITY NAME>", activity_name)
                                                text_message = text_message.replace("<AIVID BOT NAME>", BOTNAME)
                                                text_message = text_message.replace("<REGION NAME>", "Ahmedabad")
                                                text_message = text_message.replace("<AREA NAME>", "Ahmedabad")
                                                text_message = text_message.replace("<NODE NAME>", str(node_name))
                                                text_message = text_message.replace("<COUNTER NAME>", str(zone_name))
                                                text_message = text_message.replace("<SCORE>", str(score))
                                                text_message = text_message.replace("<RESPONSE>", str(responce))
                                                text_message = text_message.replace("<SENSOR NAME>", str(sensor_name))
                                                text_message = text_message.replace("<SITE NAME>", str(site_name))
                                                text_message = text_message.replace("<EVENT DATE & TIME>", str(date)+"  "+str(time_))
                                                ...
                                                print("true_notification__count ",queue_count, "notification__id__",notification_id, wait_time_, time__, start_time, diff, zone_name, botconfig_id, "real_case_flag ", real_send, real_snapshot,"pro_case_flag ",pro_send, pro_snapshot, "normal_case_flag", normal_send, normal_snapshot)
                                                if real_send == "true":
                                                    real_text_message = text_message
                                                    if real_snapshot == "true":
                                                        if _real_time_flag_json == "true":
                                                            ...
                                                            print("true_notification__send_with_snap_____real_case", queue_count, real_text_message)
                                                            self.send_notification_with_snap(to, node_name, self.camera_id, BOTNAME, _time, real_text_message)
                                                        ...
                                                        print("true_notification__send_without_snap___real_case", queue_count, real_text_message)
                                                        self.send_notification_without_snap(to, real_text_message)
                                                        self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["realtime"] = False
                                                    else:
                                                        self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["realtime"] = False
                                                else:
                                                ...
                                                if normal_send == "true":
                                                    norm_text_message = text_message
                                                    if normal_snapshot == "true":
                                                        if que_normal_flag == "true":
                                                            self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["normal"] = True
                                                            print("true_notification__send_with_snap_____normal_case", queue_count, norm_text_message)
                                                            self.send_notification_with_snap(to, node_name, self.camera_id, BOTNAME, _time, norm_text_message)
                                                        else:
                                                            self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["normal"] = False
                                                    elif que_normal_flag == "true":
                                                        self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["normal"] = False
                                                        print("true_notification__send_without_snap___normal_case", queue_count, norm_text_message)
                                                        self.send_notification_without_snap(to, norm_text_message)
                                                    else:
                                                        self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["normal"] = False
                                                else:
                                                    self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["normal"] = False
                                                ...
                                                if pro_send == "true":
                                                    ...
                                                    if pro_snapshot == "true":
                                                        if _pro_activation_flag == "true":
                                                            ...
                                                            print("true_notification__send_with_snap_____pro_case", queue_count, pro_text_message)
                                                            self.send_notification_with_snap(to, node_name, self.camera_id, BOTNAME, _time, pro_text_message)
                                                        else:
                                                            self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["proactive"] = False
                                                    elif _pro_activation_flag == "true":
                                                        print("true_notification__send_without_snap___pro_case", queue_count, pro_text_message)
                                                        self.send_notification_without_snap(to, pro_text_message)
                                                        self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["proactive"] = False
                                                    else:
                                                        self.capture[botconfig_id][self.camera_id][zone_name][notification_id]["proactive"] = False
                                                else:
...
class Capture(object):
    ...
    def capture_write_image(self,buffer, path, coordinates):
        ...
        #     with gst_buffer_data(buffer, Gst.MapFlags.READ) as data:
        #         path1=os.getcwd()
        #         print("datadatadata_try",data)

        #         data = ImageDraw.Draw(data)

        #         # a = [12,23,24,34,23,14,34,23]
        #         new_corrdinates = []
        #         for i in range(len(coordinates)-1):
        #             if i%2 == 0:

        #                 b = (coordinates[i], coordinates[i+1])
        #                 new_corrdinates.append(b)
        #         new_corrdinates = tuple(new_corrdinates)

        #         data.polygon((new_corrdinates), fill=200)
        #         # print(c)
                            ...
                            for images in list(zone.keys()):
                                ...
                                try:
                                    if image_data["image"] == True:
                                        ...
                                        self.capture_write_image(buffer, image_path, coordinates)
                                        temp_path = "http://"+static_ip+"/images/"+str(node_name)+"/"+str(sensor_id)+"/"+str(botname)+"/"+"img_"+str(time_1)+".jpeg"
