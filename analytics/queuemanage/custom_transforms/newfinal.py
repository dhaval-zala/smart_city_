    ...
    def send_notification_with_snap(self,to, json_msg,time,date, node_name, sensor_id, botname, timestamp, text_message):
        ...
        json_msg1={"eventId": "123",
            ...
            "templates":{
                ...
                "type": "WHATSAPP"}}
        print("json_msg1json_msg1json_msg1json_msg1json_msg1json_msg1",json_msg1)
#         json_msg1={
#   "eventId": "123",
#   "mqttEventName": "NOTIFICATION",
#   "templates": {
...
# }
    ...
    def process_frame(self,frame):
        ...
        for required_data in new_data["data"]:
            ...
            self.compliance_capture[notification_id] = {}
            ...
            try:
                ...
                if len(messages)>0:
                    ...
                    try:
                        ...
                        new = final_json_data(notification_id, sensor_ids, zone_names)
                        # new = True
                    except Exception as error:
                    ...
                    for i in data['sensors']:
                        ...
                        if i["url"]==json_msg['source']:
                            ...
                            self.compliance_capture[notification_id][self.camera_id] = {}
                            ...
                            for zone_name in sensor_zone:
                                ...
                                self.compliance_capture[notification_id][self.camera_id][zone_cnt] = {}
                                ...
                                for k in range(len(self.roi_box)):
                                    ...
                                    self.queue[notification_id][self.camera_id][k].queue_count = 0
                                ...
                                for m in list(detected_centers.keys()):
                                    ...
                                    try:
                                        info=self.inroi(x,y,self.queue[notification_id][self.camera_id][zone_cnt].roi)
                                    except Exception as error:
                                        print("wrong in tracker:{}".format(error),flush=True)
                                    print("infoinfoinfoinfo",info)
                                    try:
                                        if info==True:
                                            if zone_cnt not in self.queue[notification_id][self.camera_id][zone_cnt].entry:
                                                self.queue[notification_id][self.camera_id][zone_cnt].entry.append(zone_cnt)
                                                self.tracker.start_time[zone_cnt]=time      
                                            
                                            self.queue[notification_id][self.camera_id][zone_cnt].queue_count=self.queue[notification_id][self.camera_id][zone_cnt].queue_count+1
                                            self.tracker.end_time[zone_cnt]=time  
                                        
                                            
                                        else:
                                            if zone_cnt in self.queue[notification_id][self.camera_id][zone_cnt].entry and j not in self.queue[notification_id][self.camera_id][zone_cnt].exit:
                                                self.tracker.end_time[zone_cnt]=time 
                                ...
                                try:
                                    ...
                                    self.total_count[notification_id][self.camera_id][zone_cnt]=self.queue[notification_id][self.camera_id][zone_cnt].queue_count
                            

                                    self.entry_count[notification_id][self.camera_id][zone_cnt]=len(self.queue[notification_id][self.camera_id][zone_cnt].entry)
                                    self.exit_count[notification_id][self.camera_id][zone_cnt]=self.entry_count[notification_id][self.camera_id][zone_cnt]-self.total_count[notification_id][self.camera_id][j]
                                ...
                                self.compliance_capture[notification_id][self.camera_id][zone_cnt]["timestamp"] = _time
                                self.compliance_capture[notification_id][self.camera_id][zone_cnt]["node_name"] = node_name
                                self.compliance_capture[notification_id][self.camera_id][zone_cnt]["sensor_id"] = self.camera_id
                                self.compliance_capture[notification_id][self.camera_id][zone_cnt]["botname"] = "queuemanage"
                                if new == True:
                                    print("mewwww",new)
                                    ...
                                    json_msg["queueLength"]=self.total_count[notification_id][self.camera_id][zone_cnt]  
                                    ...
                                    print("mewwww__1",new)
                                    if self.total_count[notification_id][self.camera_id][zone_cnt] > int(exe) and pro_send != "true":
                                        ...
                                        self.compliance_capture[notification_id][self.camera_id][zone_cnt]["image"] = True
                                    else:
                                        json_msg["compliance"] = True 
                                        json_msg["inspectionScore"] =inspectionScore_p 
                                        self.compliance_capture[notification_id][self.camera_id][zone_cnt]["image"] = False
                                
                                    #json_msg["videoPath"] = str(path_r) 
                                    # try:
                                    #     time_stamps = []
                                    ...
                                    self.compliance_capture[notification_id][self.camera_id][zone_cnt]["image"] = "None"
                                    # print("else____")
                                ...
                                print("notification__count ",queue_count, "notification__id__",notification_id, wait_time_, time__, start_time, diff, zone_name, "real case flag ", real_send, real_snapshot,"pro case flag ",pro_send, pro_snapshot, "normal case flag", normal_send, normal_snapshot)
                                if real_send == "true":
                                ...
                                #         try:
                                #             if info==True:
                                #                 if j not in self.queue[notification_id][self.camera_id][j].entry:
                                #                     self.queue[notification_id][self.camera_id][j].entry.append(j)
                                ...
                                #             else:
                                #                 if j in self.queue[notification_id][self.camera_id][j].entry and j not in self.queue[notification_id][self.camera_id][j].exit:
                                #                     self.tracker.end_time[j]=time 
                                #                     # spend_time=datetime.datetime.strptime(self.tracker.end_time[j],'%H:%M:%S') - datetime.datetime.strptime(self.tracker.start_time[j],'%H:%M:%S')   
                                ...
                                print("queue_countqueue_countqueue_countqueue_countqueue_count",queue_count)
...
# }
    ...
    def process_frame(self,frame):
        ...
        if len(messages)>0:
            ...
            for sensor in list(compliance_capture.keys()):
                print("sensorsensor",sensor)
                try:
                    sensor = compliance_capture[sensor]
                    for zone in list(sensor.keys()):
                        zone = sensor[zone]
                        for images in list(zone.keys()):
                            image_data = zone[images]
                            try:
                                time_ = image_data["timestamp"]
                            ...
                            try:
                                if image_data["image"] == True:
                                    self.write_image(buffer, time_, node_name, sensor_id, botname)
                                    temp_path = "http://122.169.112.242/images/"+str(node_name)+"/"+str(sensor_id)+"/"+str(botname)+"/"+"img_"+str(time_)+".jpeg"
                    ...
                    print("wrong in write_image_error", err)
