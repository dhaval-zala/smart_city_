...
kind: Deployment
metadata:
  name: defn(`OFFICE_NAME')-camera-discovery-queuemanage
  labels:
     app: defn(`OFFICE_NAME')-camera-discovery-queuemanage
spec:
  replicas: 1
  selector:
    matchLabels:
        ...
        - name: defn(`OFFICE_NAME')-camera-discovery-queuemanage
          ...
          env:
            - name: PORT_SCAN
              value: "-Pn"
            - name: SIM_HOST
              value: ifelse(eval(defn(`NCAMERAS4')>0),1,"loop(`CAMERAIDX',1,defn(`NCAMERAS4'),`defn(`OFFICE_NAME')-cameras-queuemanage-service:eval(defn(`CAMERA_RTSP_PORT')+defn(`CAMERAIDX')*defn(`CAMERA_PORT_STEP')-defn(`CAMERA_PORT_STEP'))/')",":0")
            - name: SIM_PREFIX
              value: "`cams2o'defn(`OFFICEIDX')b"
            - name: OFFICE
              value: "defn(`OFFICE_LOCATION')"
            - name: DBHOST
              value: "http://ifelse(defn(`NOFFICES'),1,db,defn(`OFFICE_NAME')-db)-service:9200"
...
kind: Deployment
metadata:
  name: defn(`OFFICE_NAME')-camera-discovery-heatmap
  labels:
     app: defn(`OFFICE_NAME')-camera-discovery-heatmap
spec:
  replicas: 1
  selector:
    matchLabels:
        ...
        - name: defn(`OFFICE_NAME')-camera-discovery-heatmap
          ...
          env:
            - name: PORT_SCAN
              value: "-Pn"
            - name: SIM_HOST
              value: ifelse(eval(defn(`NCAMERAS5')>0),1,"loop(`CAMERAIDX',1,defn(`NCAMERAS5'),`defn(`OFFICE_NAME')-cameras-heatmap-service:eval(defn(`CAMERA_RTSP_PORT')+defn(`CAMERAIDX')*defn(`CAMERA_PORT_STEP')-defn(`CAMERA_PORT_STEP'))/')",":0")
            - name: SIM_PREFIX
              value: "`cams2o'defn(`OFFICEIDX')d"
            - name: OFFICE
              value: "defn(`OFFICE_LOCATION')"
            - name: DBHOST
              value: "http://ifelse(defn(`NOFFICES'),1,db,defn(`OFFICE_NAME')-db)-service:9200"
